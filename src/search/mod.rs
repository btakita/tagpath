...
pub fn search(query: &str, path: &Path) -> Vec<SearchResult> {
    ...
    let query_tags: Vec<&str> = query_parsed.tags.iter().map(|s| s.as_str()).collect();
...
mod tests {
    ...
    fn test_cross_language_search() {
        ...
        let dir = std::env::temp_dir().join("tagpath_test_cross_lang");
        let _ = std::fs::create_dir_all(&dir);
        ...
        // Go: PascalCase
        {
            let mut f = std::fs::File::create(dir.join("auth.go")).unwrap();
            writeln!(f, "func ValidateUser(name string) error {{}}").unwrap();
        }
        // Rust: snake_case
        {
            let mut f = std::fs::File::create(dir.join("auth.rs")).unwrap();
            writeln!(f, "fn validate_user(name: &str) {{}}").unwrap();
        }
        // CSS: kebab-case
        {
            let mut f = std::fs::File::create(dir.join("auth.css")).unwrap();
        ...
        let idents2: Vec<&str> = results2.iter().map(|r| r.identifier.as_str()).collect();
        assert!(
        ...
        let idents3: Vec<&str> = results3.iter().map(|r| r.identifier.as_str()).collect();
        assert!(
    ...
    fn test_cross_language_partial_tag_match() {
        ...
        let dir = std::env::temp_dir().join("tagpath_test_cross_lang_partial");
        let _ = std::fs::create_dir_all(&dir);

        // Different languages, different conventions, all contain "user"
        {
            let mut f = std::fs::File::create(dir.join("models.py")).unwrap();
            writeln!(f, "def create_user_profile():").unwrap();
            writeln!(f, "    pass").unwrap();
        }
        {
            let mut f = std::fs::File::create(dir.join("api.ts")).unwrap();
            writeln!(f, "function fetchUserData() {{}}").unwrap();
            writeln!(f, "function fetchPostData() {{}}").unwrap();
        }
        {
            let mut f = std::fs::File::create(dir.join("handler.go")).unwrap();
