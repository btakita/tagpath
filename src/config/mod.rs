...
fn load_extends_entry(entry: &str, config_dir: &Path) -> Result<NamingConfig, String> {
    // Try built-in language/preset first
    if let Some(content) = resolve_builtin(entry) {
        return toml::from_str(content)
            .map_err(|e| format!("failed to parse built-in '{entry}': {e}"));
    }
...
fn resolve_builtin(name: &str) -> Option<&'static str> {
    match name {
        ...
        "cpp" | "c++" | "cxx" => Some(include_str!("../../lang/cpp.toml")),
        "csharp" | "cs" | "c#" => Some(include_str!("../../lang/csharp.toml")),
        "clojure" | "clj" => Some(include_str!("../../lang/clojure.toml")),
        "common-lisp" | "cl" | "lisp" => Some(include_str!("../../lang/common-lisp.toml")),
        "crystal" | "cr" => Some(include_str!("../../lang/crystal.toml")),
        "css" => Some(include_str!("../../lang/css.toml")),
        "d" | "dlang" => Some(include_str!("../../lang/d.toml")),
        "dart" => Some(include_str!("../../lang/dart.toml")),
        "elixir" | "ex" => Some(include_str!("../../lang/elixir.toml")),
        "erlang" | "erl" => Some(include_str!("../../lang/erlang.toml")),
        "fsharp" | "fs" | "f#" => Some(include_str!("../../lang/fsharp.toml")),
        "gleam" => Some(include_str!("../../lang/gleam.toml")),
        "go" => Some(include_str!("../../lang/go.toml")),
        "haskell" | "hs" => Some(include_str!("../../lang/haskell.toml")),
        "java" => Some(include_str!("../../lang/java.toml")),
        "javascript" | "js" => Some(include_str!("../../lang/javascript.toml")),
        "julia" | "jl" => Some(include_str!("../../lang/julia.toml")),
        "kotlin" | "kt" => Some(include_str!("../../lang/kotlin.toml")),
        "lua" => Some(include_str!("../../lang/lua.toml")),
        "nim" => Some(include_str!("../../lang/nim.toml")),
        "objective-c" | "objc" => Some(include_str!("../../lang/objective-c.toml")),
        "ocaml" | "ml" => Some(include_str!("../../lang/ocaml.toml")),
        "odin" => Some(include_str!("../../lang/odin.toml")),
        "perl" | "pl" => Some(include_str!("../../lang/perl.toml")),
        "php" => Some(include_str!("../../lang/php.toml")),
        "python" | "py" => Some(include_str!("../../lang/python.toml")),
        "r" => Some(include_str!("../../lang/r.toml")),
        "racket" | "rkt" => Some(include_str!("../../lang/racket.toml")),
        "ruby" | "rb" => Some(include_str!("../../lang/ruby.toml")),
        "rust" | "rs" => Some(include_str!("../../lang/rust.toml")),
        "scala" => Some(include_str!("../../lang/scala.toml")),
        "scheme" | "scm" => Some(include_str!("../../lang/scheme.toml")),
        "shell" | "sh" | "bash" | "zsh" => Some(include_str!("../../lang/shell.toml")),
        "sql" => Some(include_str!("../../lang/sql.toml")),
        "swift" => Some(include_str!("../../lang/swift.toml")),
        "typescript" | "ts" => Some(include_str!("../../lang/typescript.toml")),
        "v" | "vlang" => Some(include_str!("../../lang/v.toml")),
        "zig" => Some(include_str!("../../lang/zig.toml")),
        // Presets
        "immutable-tag" => Some(include_str!("../../presets/immutable-tag.toml")),
        _ => None,
    }
}
...
pub fn generate_config(lang: Option<&str>, preset: Option<&str>) -> String {
    match (lang, preset) {
        (Some("typescript") | Some("ts"), _) => {
            include_str!("../../lang/typescript.toml").to_string()
        }
        (Some("python") | Some("py"), _) => include_str!("../../lang/python.toml").to_string(),
        (Some("rust") | Some("rs"), _) => include_str!("../../lang/rust.toml").to_string(),
        (Some("javascript") | Some("js"), _) => {
            include_str!("../../lang/javascript.toml").to_string()
        }
        (Some("go"), _) => include_str!("../../lang/go.toml").to_string(),
        (Some("java"), _) => include_str!("../../lang/java.toml").to_string(),
        (Some("ruby") | Some("rb"), _) => include_str!("../../lang/ruby.toml").to_string(),
        (Some("swift"), _) => include_str!("../../lang/swift.toml").to_string(),
        (Some("kotlin") | Some("kt"), _) => include_str!("../../lang/kotlin.toml").to_string(),
        (Some("c"), _) => include_str!("../../lang/c.toml").to_string(),
        ...
        (Some("php"), _) => include_str!("../../lang/php.toml").to_string(),
        (Some("elixir") | Some("ex"), _) => include_str!("../../lang/elixir.toml").to_string(),
        (Some("css"), _) => include_str!("../../lang/css.toml").to_string(),
        (Some("sql"), _) => include_str!("../../lang/sql.toml").to_string(),
        (Some("shell") | Some("sh") | Some("bash") | Some("zsh"), _) => {
            include_str!("../../lang/shell.toml").to_string()
        }
        (Some("zig"), _) => include_str!("../../lang/zig.toml").to_string(),
        (Some("odin"), _) => include_str!("../../lang/odin.toml").to_string(),
        (Some("nim"), _) => include_str!("../../lang/nim.toml").to_string(),
        (Some("haskell") | Some("hs"), _) => include_str!("../../lang/haskell.toml").to_string(),
        (Some("d") | Some("dlang"), _) => include_str!("../../lang/d.toml").to_string(),
        (Some("lua"), _) => include_str!("../../lang/lua.toml").to_string(),
        (Some("perl") | Some("pl"), _) => include_str!("../../lang/perl.toml").to_string(),
        (Some("clojure") | Some("clj"), _) => include_str!("../../lang/clojure.toml").to_string(),
        (Some("r"), _) => include_str!("../../lang/r.toml").to_string(),
        (Some("scala"), _) => include_str!("../../lang/scala.toml").to_string(),
        (Some("dart"), _) => include_str!("../../lang/dart.toml").to_string(),
        // New languages
        (Some("common-lisp") | Some("cl") | Some("lisp"), _) => {
            include_str!("../../lang/common-lisp.toml").to_string()
        }
        (Some("scheme") | Some("scm"), _) => include_str!("../../lang/scheme.toml").to_string(),
        (Some("racket") | Some("rkt"), _) => include_str!("../../lang/racket.toml").to_string(),
        (Some("erlang") | Some("erl"), _) => include_str!("../../lang/erlang.toml").to_string(),
        (Some("fsharp") | Some("fs") | Some("f#"), _) => {
            include_str!("../../lang/fsharp.toml").to_string()
        }
        (Some("ocaml") | Some("ml"), _) => include_str!("../../lang/ocaml.toml").to_string(),
        (Some("julia") | Some("jl"), _) => include_str!("../../lang/julia.toml").to_string(),
        (Some("objective-c") | Some("objc"), _) => {
            include_str!("../../lang/objective-c.toml").to_string()
        }
        (Some("v") | Some("vlang"), _) => include_str!("../../lang/v.toml").to_string(),
        (Some("crystal") | Some("cr"), _) => include_str!("../../lang/crystal.toml").to_string(),
        (Some("gleam"), _) => include_str!("../../lang/gleam.toml").to_string(),
        (_, Some("immutable-tag")) => include_str!("../../presets/immutable-tag.toml").to_string(),
        _ => default_config(),
    }
}
...
mod tests {
    ...
    fn test_resolve_extends_builtin() {
        let dir = std::env::temp_dir().join("tagpath_test_resolve_extends");
...
"#
        ...
        assert_eq!(contexts.get("function").unwrap().convention, "camelCase");
        // Type convention inherited from Rust base (PascalCase)
        assert_eq!(contexts.get("type").unwrap().convention, "PascalCase");
        // Variable convention inherited from Rust base
        assert_eq!(contexts.get("variable").unwrap().convention, "snake_case");
        let _ = std::fs::remove_dir_all(&dir);
    }

    #[test]
    fn test_resolve_no_extends() {
        let dir = std::env::temp_dir().join("tagpath_test_resolve_none");
        let _ = std::fs::create_dir_all(&dir);
        let config_path = dir.join(".naming.toml");
        {
            let mut f = std::fs::File::create(&config_path).unwrap();
...
"#
    ...
    fn test_resolve_extends_relative_path() {
        let dir = std::env::temp_dir().join("tagpath_test_resolve_rel");
        let _ = std::fs::create_dir_all(&dir);
...
"#
    ...
    fn test_resolve_extends_chain() {
        let dir = std::env::temp_dir().join("tagpath_test_resolve_chain");
        let _ = std::fs::create_dir_all(&dir);
...
"#
        ...
        let contexts = resolved.contexts.unwrap();
        assert_eq!(contexts.get("constant").unwrap().convention, "camelCase");
    ...
    fn test_merge_patterns() {
        let dir = std::env::temp_dir().join("tagpath_test_merge_pat");
        let _ = std::fs::create_dir_all(&dir);
        let base_path = dir.join("base.toml");
        {
            let mut f = std::fs::File::create(&base_path).unwrap();
