...
const KEBAB_EXTENSIONS: &[&str] = &["css", "lisp", "lsp", "clj", "scm", "rkt"];
...
const KEYWORDS: &[&str] = &[
    "if", "else", "for", "while", "return", "fn", "func", "function", "def", "class", "struct",
    "enum", "impl", "use", "import", "from", "let", "const", "var", "pub", "self", "this", "true",
    "false", "null", "none", "nil",
];
...
pub fn extract_from_path_with_mode(path: &Path, ast_mode: bool) -> Vec<ExtractedIdentifier> {
    if path.is_file() {
        return if ast_mode {
        ...
        .filter_entry(|e| !is_skipped_dir(e))
    {
        ...
        if ast_mode {
            results.extend(extract_from_file_ast(entry.path()));
        } else {
...
fn extract_from_file_ast(path: &Path) -> Vec<ExtractedIdentifier> {
    ...
    contextual
        .into_iter()
        .map(|ci| {
            let convention = parser::detect_convention(&ci.identifier);
            let parsed = parser::parse(&ci.identifier, convention);
            ExtractedIdentifier {
                file: ci.file,
                line: ci.line,
                column: ci.column,
...
mod tests {
    ...
    fn test_extract_ast_mode() {
        ...
        let results = extract_from_path_with_mode(&file, true);
        // AST mode should populate context
        let fns: Vec<&str> = results
            .iter()
            .filter(|r| r.context == Some(IdentifierContext::Function))
            .map(|r| r.identifier.as_str())
            .collect();
        assert!(fns.contains(&"create_user"));
        let params: Vec<&str> = results
            .iter()
            .filter(|r| r.context == Some(IdentifierContext::Parameter))
            .map(|r| r.identifier.as_str())
            .collect();
    ...
    fn test_extract_ast_fallback_unsupported() {
        let dir = std::env::temp_dir().join("tagpath_test_ast_fallback");
        let _ = std::fs::create_dir_all(&dir);
        let file = dir.join("style.css");
        {
            let mut f = std::fs::File::create(&file).unwrap();
