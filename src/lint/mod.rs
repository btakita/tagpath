...
pub fn lint(path: &Path, config: &NamingConfig) -> Vec<LintViolation> {
    // Extract identifiers with AST context
    let identifiers = extract::extract_from_path_with_mode(path, true);
...
mod tests {
    ...
    fn test_suggest_fix_snake_case() {
        let tags = vec!["parse".to_string(), "name".to_string()];
        assert_eq!(
    ...
    fn test_suggest_fix_camel_case() {
        let tags = vec!["parse".to_string(), "name".to_string()];
        assert_eq!(parser::join_tags(&tags, Convention::CamelCase), "parseName");
    }

    #[test]
    fn test_suggest_fix_upper_snake() {
        let tags = vec!["max".to_string(), "retries".to_string()];
        assert_eq!(
    ...
    fn test_lint_finds_violations() {
        ...
        let file = dir.join("bad.rs");
        {
            ...
            writeln!(f, "fn ParseName(user_input: &str) {{}}").unwrap();
        }
        ...
        let flagged: Vec<&str> = violations.iter().map(|v| v.identifier.as_str()).collect();
        assert!(
        ...
        let violation = violations
            ...
            .unwrap();
        assert_eq!(violation.suggested_fix, Some("parse_name".to_string()));
    ...
    fn test_lint_no_violations() {
        let dir = std::env::temp_dir().join("tagpath_test_lint_ok");
        let _ = std::fs::create_dir_all(&dir);
        let file = dir.join("good.rs");
        {
            let mut f = std::fs::File::create(&file).unwrap();
            writeln!(f, "fn parse_name(user_input: &str) {{}}").unwrap();
        }
        let config = make_config(vec![
            ("function", "snake_case"),
            ("parameter", "snake_case"),
    ...
    #[test]
    fn test_find_config() {
        let dir = std::env::temp_dir().join("tagpath_test_find_config");
        let _ = std::fs::create_dir_all(&dir);
    ...
    fn test_lint_type_convention() {
        let dir = std::env::temp_dir().join("tagpath_test_lint_type");
        let _ = std::fs::create_dir_all(&dir);
        ...
        let config = make_config(vec![("type", "PascalCase")]);
        let violations = lint(&file, &config);
        let flagged: Vec<&str> = violations.iter().map(|v| v.identifier.as_str()).collect();
        assert!(
