...
- The fixture covers four tsift preview surfaces: `search`, `explain`, `session-review`, and `context-pack`.
...
- Tests must verify that raw symbols parse back to the fixture family counts through the shared compression-report helper, aliases are generated by tagpath rather than hand-maintained drift, `context-pack` carries ontology/summary/artifact handles, and compact previews satisfy each case's savings threshold.
